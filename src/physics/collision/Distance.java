...
public class Distance {
	...
	private class SimplexVertex {
		public final Vec wA = new Vec(); // support point in shapeA
		public final Vec wB = new Vec(); // support point in shapeB
		public final Vec w = new Vec(); // wB - wA
	...
	private class Simplex {
		...
		public void readCache(SimplexCache cache, DistanceProxy proxyA, Transform transformA, DistanceProxy proxyB,
			...
			for (int i = 0; i < m_count; ++i) {
				...
				Vec wALocal = proxyA.getVertex(v.indexA);
				Vec wBLocal = proxyB.getVertex(v.indexB);
			...
			if (m_count == 0) {
				...
				v.indexA = 0;
				v.indexB = 0;
				Vec wALocal = proxyA.getVertex(0);
				Vec wBLocal = proxyB.getVertex(0);
				Transform.mulToOut(transformA, wALocal, v.wA);
				Transform.mulToOut(transformB, wBLocal, v.wB);
				v.w.set(v.wB).subLocal(v.wA);
				m_count = 1;
		...
		private final Vec e12 = new Vec();
		
		public final void getSearchDirection(final Vec out) {
			switch (m_count) {
				case 1 :
				...
				case 2 :
					...
					out.set(m_v1.w).negateLocal();
					float sgn = Vec.cross(e12, out);
					
					if (sgn > 0f) {
						// Origin is left of e12.
						Vec.crossToOut(1f, e12, out);
						return;
					}
					else {
						// Origin is right of e12.
						Vec.crossToOut(e12, 1f, out);
						return;
					}
				default :
					assert (false);
		...
		// djm pooled
		private final Vec case2 = new Vec();
		private final Vec case22 = new Vec();
		 ...
		 * @return
		 */
		public void getClosestPoint(final Vec out) {
			switch (m_count) {
				case 0 :
					assert (false);
					out.setZero();
		...
		private final Vec case3 = new Vec();
		private final Vec case33 = new Vec();
		
		public void getWitnessPoints(Vec pA, Vec pB) {
			switch (m_count) {
				case 0 :
					assert (false);
					break;
		...
		// djm pooled, and from above
		private final Vec e13 = new Vec();
		private final Vec e23 = new Vec();
		private final Vec w1 = new Vec();
		private final Vec w2 = new Vec();
		private final Vec w3 = new Vec();
		...
		public void solve3() {
			...
			// a3 = 0
			e12.set(w2).subLocal(w1);
			float w1e12 = Vec.dot(w1, e12);
			float w2e12 = Vec.dot(w2, e12);
			float d12_1 = w2e12;
			float d12_2 = -w1e12;
			...
			float w1e13 = Vec.dot(w1, e13);
			float w3e13 = Vec.dot(w3, e13);
			float d13_1 = w3e13;
			float d13_2 = -w1e13;
			...
			float w2e23 = Vec.dot(w2, e23);
			float w3e23 = Vec.dot(w3, e23);
			float d23_1 = w3e23;
			float d23_2 = -w2e23;
			
			// Triangle123
			float n123 = Vec.cross(e12, e13);
			
			float d123_1 = n123 * Vec.cross(w2, w3);
			float d123_2 = n123 * Vec.cross(w3, w1);
			float d123_3 = n123 * Vec.cross(w1, w2);
	...
	public static class DistanceProxy {
		 ...
		 * @return
		 */
		public final Vec getVertex(int index){
	...
	private Vec closestPoint = new Vec();
	private Vec d = new Vec();
	private Vec temp = new Vec();
