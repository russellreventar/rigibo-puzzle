...
import static physics.collision.broadphase.DynamicTreeNode.NULL_NODE;

import physics.collision.AABB;
import physics.collision.TreeCallback;
import physics.dataStorage.DynamicIntStack;
...
public class DynamicTree {
  ...
  private final Vec[] drawVecs = new Vec[4];
   ...
   */
  public final boolean moveProxy(DynamicTreeNode proxy, final AABB aabb, Vec displacement) {
	assert(proxy != null);
	int proxyId = proxy.id;
    assert (0 <= proxyId && proxyId < m_nodeCapacity);
    final DynamicTreeNode node = m_nodes[proxyId];
    ...
    // Extend AABB
    final Vec lowerBound = aabb.lowerBound;
    final Vec upperBound = aabb.upperBound;
