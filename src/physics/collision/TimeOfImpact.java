...
class SeparationFunction {
	...
	public Type m_type;
	public final Vec m_localPoint = new Vec();
	public final Vec m_axis = new Vec();
	public Sweep m_sweepA;
	public Sweep m_sweepB;
	
	// djm pooling
	private final Vec localPointA = new Vec();
	private final Vec localPointB = new Vec();
	private final Vec pointA = new Vec();
	private final Vec pointB = new Vec();
	private final Vec localPointA1 = new Vec();
	private final Vec localPointA2 = new Vec();
	private final Vec normal = new Vec();
	private final Vec localPointB1 = new Vec();
	private final Vec localPointB2 = new Vec();
	...
	public float initialize(final SimplexCache cache, final DistanceProxy proxyA, final Sweep sweepA,
		...
		if (count == 1) {
			...
			Vec.crossToOut(temp, 1.0f, m_axis);
			m_axis.normalize();
	...
	private final Vec axisA = new Vec();
	private final Vec axisB = new Vec();
	...
	public float findMinSeparation(int[] indexes, float t) {
		...
		switch (m_type) {
			case POINTS : {
				...
				float separation = Vec.dot(pointB.subLocal(pointA), m_axis);
				return separation;
			}
			case FACE_A : {
				Mat22.mulToOut(xfa.R, m_axis, normal);
				...
				float separation = Vec.dot(pointB.subLocal(pointA), normal);
				return separation;
			}
			case FACE_B : {
				Mat22.mulToOut(xfb.R, m_axis, normal);
				...
				float separation = Vec.dot(pointA.subLocal(pointB), normal);
				return separation;
			}
			default :
				assert (false);
